...
- **Full technical spec** (schemas, thresholds, build phases, data contracts): [`docs/SPEC.md`](./docs/SPEC.md)
...
All tunables live in [`config/tmq.yaml`](./config/tmq.yaml) (mounted into the worker). Secrets and connection strings come from the environment (12-factor).
...
Full schemas in [`docs/SPEC.md §4`](./docs/SPEC.md). Highlights:
...
*Architecture, data contracts, and build phases in full: [`docs/SPEC.md`](./docs/SPEC.md). The prior single-file architecture note is superseded — see [`SYSTEM_ARCHITECTURE.md`](./SYSTEM_ARCHITECTURE.md).*
